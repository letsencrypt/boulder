...
func (t *Throughput) optimizeAndValidate() error {
	...
	if t.PurgeBatchInterval.Get() < 0 {
		return errors.New("'purgeBatchInterval' must be positive or 0 (for the default)")
	} else if t.PurgeBatchInterval.Get() == 0 {
		t.PurgeBatchInterval.Set(defaultPurgeBatchInterval * time.Duration(t.TotalInstances))
	}

	// Send no more than the 50 API requests we’re allotted each second.
	requestsPerSecond := int(math.Ceil(float64(time.Second)/float64(t.PurgeBatchInterval.Get()))) * t.TotalInstances
...
func daemon(c Config, ap *akamaiPurger, logger blog.Logger, scope prometheus.Registerer) {
	...
	ticker := time.NewTicker(c.AkamaiPurger.Throughput.PurgeBatchInterval.Get())
	go func() {
	loop:
		for {
			select {
