...
func parseLogLine(sa certificateStorage, logger blog.Logger, line string) (found bool, added bool) {
	...
	if len(derStr) <= 1 {
		logger.AuditErr(fmt.Errorf("Didn't match regex for b64der: %s", line))
		return true, false
	}
	der, err := base64.StdEncoding.DecodeString(derStr[1])
	if err != nil {
		logger.AuditErr(fmt.Errorf("Couldn't decode b64: %s, [%s]", err, line))
		return true, false
	}
	err = checkDER(sa, der)
	if err != nil {
	...
	if len(regStr) <= 1 {
		logger.AuditErr(fmt.Errorf("regID variable is empty, [%s]", line))
		return true, false
	}
	regID, err := strconv.Atoi(regStr[1])
	if err != nil {
		logger.AuditErr(fmt.Errorf("Couldn't parse regID: %s, [%s]", err, line))
		return true, false
	}
	_, err = sa.AddCertificate(ctx, der, int64(regID))
	if err != nil {
		logger.AuditErr(fmt.Errorf("Failed to store certificate: %s, [%s]", err, line))
		return true, false
	}
	return true, true
}
