...
func TestSearch(t *testing.T) {
	...
	derB, err := x509.CreateCertificate(rand.Reader, &temp, &temp, &k2.PublicKey, k2)
	test.AssertNotError(t, err, "x509.CreateCertificate failed")
	...
	derC, err := x509.CreateCertificate(rand.Reader, &temp, &temp, &wk.PublicKey, wk)
	test.AssertNotError(t, err, "x509.CreateCertificate failed")
	...
	err = dbMap.Insert(
		...
		&core.Certificate{RegistrationID: reg.ID, Serial: "cert-d", DER: derC, Issued: now},
		&core.Certificate{RegistrationID: reg.ID, Serial: "cert-e", DER: derA, Issued: now.Add(time.Hour * 2)},
	)
	...
	test.AssertDeepEquals(t, log.GetAllMatching("INFO: cert contains weak key: .*"), []string{"INFO: cert contains weak key: cert-d"})
}
