...
func (m *mailer) updateCertStatus(serial string) error {
	...
	if err != nil {
		...
		m.log.AuditErr(fmt.Errorf("Error opening transaction for certificate %s: %s", serial, err))
		return err
	}
	...
	if err != nil {
		err = sa.Rollback(tx, err)
		m.log.AuditErr(fmt.Errorf("Error fetching status for certificate %s: %s", serial, err))
		return err
	}
	...
	if err != nil {
		err = sa.Rollback(tx, err)
		m.log.AuditErr(fmt.Errorf("Error updating status for certificate %s: %s", serial, err))
		return err
	}
	...
	if err != nil {
		err = sa.Rollback(tx, err)
		m.log.AuditErr(fmt.Errorf("Error committing transaction for certificate %s: %s", serial, err))
		return err
	}
...
func (m *mailer) processCerts(allCerts []core.Certificate) {
	...
	for regID, certs := range regIDToCerts {
		...
		if err != nil {
			m.log.AuditErr(fmt.Errorf("Error fetching registration %d: %s", regID, err))
		...
		for _, cert := range certs {
			...
			if err != nil {
				...
				m.log.AuditErr(fmt.Errorf("Error parsing certificate %s: %s", cert.Serial, err))
			...
			if err != nil {
				m.log.AuditErr(fmt.Errorf("expiration-mailer: error fetching renewal state: %v", err))
				// assume not renewed
			} else if renewed {
		...
		if err != nil {
			m.log.AuditErr(fmt.Errorf("Error sending nag emails: %s", err))
			continue
		}
		for _, cert := range parsedCerts {
			...
			if err != nil {
				m.log.AuditErr(fmt.Errorf("Error updating certificate status for %s: %s", serial, err))
...
func (m *mailer) findExpiringCertificates() error {
	...
	for i, expiresIn := range m.nagTimes {
		...
		if err != nil {
			m.log.AuditErr(fmt.Errorf("expiration-mailer: Error loading certificates: %s", err))
			return err // fatal
		}
