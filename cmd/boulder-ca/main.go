...
func main() {
	...
	if c.CA.LifespanOCSP.Get() == 0 {
		c.CA.LifespanOCSP.Set(96 * time.Hour)
	}
	...
	if c.CA.Issuance.CRLProfile.ValidityInterval.Get() == 0 && c.CA.LifespanCRL.Get() != 0 {
		c.CA.Issuance.CRLProfile.ValidityInterval = c.CA.LifespanCRL
	}
	if c.CA.Issuance.CRLProfile.MaxBackdate.Get() == 0 && c.CA.Backdate.Get() != 0 {
	...
	if !c.CA.DisableOCSPService {
		ocspi, err := ca.NewOCSPImpl(
			issuers,
			c.CA.LifespanOCSP.Get(),
			c.CA.OCSPLogMaxLength,
			c.CA.OCSPLogPeriod.Get(),
			logger,
			scope,
			metrics,
			clk,
