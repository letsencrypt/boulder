...
func main() {
	...
	saroi, err := sa.NewSQLStorageAuthorityRO(
		dbReadOnlyMap, dbIncidentsMap, scope, parallel, c.SA.LagFactor.Get(), clk, logger)
	...
	start, err := bgrpc.NewServer(c.SA.GRPC, logger).WithCheckInterval(c.SA.HealthCheckInterval.Get()).Add(
