...
`, os.Args[0])
	...
	if strings.HasPrefix(config.Source, "file:") {
		...
		// Collect and periodically report DB metrics using the DBMap and prometheus stats registerer.
	...
	stats.MustRegister(responseTypesCounter)

	m := mux(stats, c.OCSPResponder.Path, source, responseTypesCounter)
	srv := &http.Server{
...
func mux(stats prometheus.Registerer, responderPath string, source bocsp.Source, responseTypesCounter *prometheus.CounterVec) http.Handler {
	stripPrefix := http.StripPrefix(responderPath, bocsp.NewResponder(source, responseTypesCounter))
