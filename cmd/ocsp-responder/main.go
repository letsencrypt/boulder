...
`, os.Args[0])
	...
	if strings.HasPrefix(config.Source, "file:") {
		...
		if c.OCSPResponder.Redis.Addrs != nil {
			...
			dbsource, err = responder.NewMultiSource(dbsource, rsource, stats, logger)
...
func mux(responderPath string, source ocsp_responder.Source, timeout time.Duration, stats prometheus.Registerer, logger blog.Logger) http.Handler {
	stripPrefix := http.StripPrefix(responderPath, ocsp_responder.NewResponder(source, timeout, stats, logger))
