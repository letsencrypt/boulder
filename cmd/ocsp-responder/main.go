...
`, os.Args[0])
	...
	if strings.HasPrefix(c.OCSPResponder.Source, "file:") {
		...
		liveSigningPeriod := c.OCSPResponder.LiveSigningPeriod.GetDuration()
	...
	m := mux(c.OCSPResponder.Path, source, c.OCSPResponder.Timeout.GetDuration(), scope, c.OpenTelemetryHTTPConfig.Options(), logger, c.OCSPResponder.LogSampleRate)
	...
	defer func() {
		ctx, cancel := context.WithTimeout(context.Background(),
			c.OCSPResponder.ShutdownStopTimeout.GetDuration())
		defer cancel()
		_ = srv.Shutdown(ctx)
		oTelShutdown(ctx)
	}()
