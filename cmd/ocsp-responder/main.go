...
func (f *ocspFilter) checkRequest(req *ocsp.Request) error {
	if req.HashAlgorithm != f.issuerKeyHashAlgorithm {
		return fmt.Errorf("Request ca key hash using unsupported algorithm %s: %w", req.HashAlgorithm, ocsp_responder.ErrNotFound)
	}
	...
	if !match {
		return fmt.Errorf("Request intended for wrong issuer cert %s: %w", hex.EncodeToString(req.IssuerKeyHash), ocsp_responder.ErrNotFound)
	}
	...
	if len(f.serialPrefixes) > 0 {
		...
		if !match {
			return fmt.Errorf("Request serial has wrong prefix: %w", ocsp_responder.ErrNotFound)
		}
	}
...
// Response implements the `ocsp_responder.Source` interface and is called by
...
func (src *dbSource) Response(ctx context.Context, req *ocsp.Request) ([]byte, http.Header, error) {
	...
	case primaryResult := <-primaryChan:
		if primaryResult.err != nil {
			if !errors.Is(primaryResult.err, ocsp_responder.ErrNotFound) {
...
func (src dbReceiver) getResponse(ctx context.Context, req *ocsp.Request) chan lookupResponse {
	...
	go func() {
		...
		if err != nil {
			if db.IsNoRows(err) {
				responseChan <- lookupResponse{nil, ocsp_responder.ErrNotFound}
				return
			}
...
`, os.Args[0])
	...
	var source ocsp_responder.Source
	...
	if strings.HasPrefix(config.Source, "file:") {
		...
		source, err = ocsp_responder.NewMemorySourceFromFile(filename, logger)
...
func mux(stats prometheus.Registerer, responderPath string, source ocsp_responder.Source, logger blog.Logger) http.Handler {
	stripPrefix := http.StripPrefix(responderPath, ocsp_responder.NewResponder(source, stats, logger))
