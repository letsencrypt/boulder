...
`, os.Args[0])
	...
	m := mux(stats, c.OCSPResponder.Path, source, stats)
	srv := &http.Server{
...
func mux(stats prometheus.Registerer, responderPath string, source bocsp.Source, stats prometheus.Registerer) http.Handler {
	stripPrefix := http.StripPrefix(responderPath, bocsp.NewResponder(source, stats))
