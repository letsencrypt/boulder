...
`, os.Args[0])
	...
	if strings.HasPrefix(config.Source, "file:") {
		...
		dbsource, err := responder.NewDbSource(dbMap, stats, logger)
		...
		if c.OCSPResponder.Redis.Addrs != nil {
			...
			rsource, err := responder.NewRedisSource(rocspReader, stats, logger)
			cmd.FailOnError(err, "Could not create redis source")

			dbsource, err = responder.NewMultiSource(dbsource, rsource, c.OCSPResponder.Timeout.Duration, stats, logger)
