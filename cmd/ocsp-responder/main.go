...
`, os.Args[0])
	...
	m := mux(c.OCSPResponder.Path,
		source,
		c.OCSPResponder.Timeout.Duration,
		c.OCSPResponder.RAService.Timeout.Duration,
		scope,
		logger,
		c.OCSPResponder.LogSampleRate)
...
func mux(responderPath string,
	...
	return hnynethttp.WrapHandler(measured_http.New(&ocspMux{h}, cmd.Clock(), stats, buckets))
}
