...
`, os.Args[0])
	...
	if strings.HasPrefix(config.Source, "file:") {
		...
		primary, err := responder.NewDbSource(dbMap, logger)
		...
		var secondary responder.Source
		if c.OCSPResponder.Redis.Addrs != nil {
			...
			secondary, err = responder.NewRedisSource(rocspReader, logger)
			cmd.FailOnError(err, "Could not create redis source")
		} else {
			logger.Info("no redis config found, using mysql as only ocsp source")
		}

		multi, err := responder.NewMultiSource(primary, secondary, c.OCSPResponder.Timeout.Duration, logger)
		...
		for i, issuerFile := range c.OCSPResponder.IssuerCerts {
			...
			cmd.FailOnError(err, "Cound not load issuer cert")
