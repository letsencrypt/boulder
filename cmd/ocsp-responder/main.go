...
// ocspMux implements the Handler method exposed by http.ServeMux which we use in the WFE
// but removes all of the internal path cleaning stuff it does as it can break OCSP requests.
// Notably http.ServeMux will collapse repeated slashes into a single slash which breaks
// the base64 standard encoding that is used in OCSP GET requests. CFSSL explicitly
// recommends against using http.ServeMux for this reason:
