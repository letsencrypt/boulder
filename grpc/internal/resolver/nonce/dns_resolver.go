...
var logger = grpclog.Component("noncesrv")
...
// Scheme returns the naming scheme of this resolver builder, which is "noncesrv".
	...
	for _, n := range d.names {
		...
		for _, s := range srvs {
			...
			for _, a := range backendAddrs {
				ip, ok := formatIP(a)
				if !ok {
					return nil, fmt.Errorf("noncesrv: error parsing A record IP address %v", a)
				}
				...
				// TODO: We must derive the nonce prefix here but we don't have the
				// right salt string to produce it yet. The Nonce prefix will be the
				// first 24 bytes of the SHA256 hash of the IP address and the salt.
				// if noncePrefix != "" {
				// 	resolverAddr.Attributes = attributes.New("nonce_prefix", noncePrefix)
				// }
				newAddrs = append(newAddrs, resolverAddr)
			}
		}
	}
	return newAddrs, nil
...
func handleDNSError(err error, lookupType string) error {
	...
	if err != nil {
		err = fmt.Errorf("noncesrv: %v record lookup error: %v", lookupType, err)
		logger.Info(err)
	}
	return err
}
