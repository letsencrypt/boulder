...
func (si *serverInterceptor) intercept(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
	if info == nil {
		si.stats.Inc("gRPCServer.NoInfo", 1)
		return nil, errors.New("passed nil *grpc.UnaryServerInfo")
	}
	s := si.clk.Now()
	si.stats.Inc(fmt.Sprintf("gRPCServer.%s", info.FullMethod), 1)
	si.stats.GaugeDelta(fmt.Sprintf("gRPCServer.%s.InProgress", info.FullMethod), 1)
	resp, err := handler(ctx, req)
	si.stats.TimingDuration(fmt.Sprintf("gRPCServer.%s", info.FullMethod), si.clk.Since(s))
	si.stats.GaugeDelta(fmt.Sprintf("gRPCServer.%s.InProgress", info.FullMethod), -1)
	if err != nil {
		si.stats.Inc(fmt.Sprintf("gRPCServer.%s.Failed", info.FullMethod), 1)
	}
	return resp, err
}
...
func (ci *clientInterceptor) intercept(ctx context.Context, method string, req, reply interface{}, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
	s := ci.clk.Now()
	ci.stats.Inc(fmt.Sprintf("gRPCClient.%s", method), 1)
	flight := fmt.Sprintf("gRPCClient.%s.InProgress", method)
	ci.stats.GaugeDelta(flight, 1)
	err := invoker(ctx, method, req, reply, cc, opts...)
	ci.stats.TimingDuration(fmt.Sprintf("gRPCClient.%s", method), ci.clk.Since(s))
	ci.stats.GaugeDelta(flight, -1)
	if err != nil {
		ci.stats.Inc(fmt.Sprintf("gRPCClient.%s.Failed", method), 1)
	}
	return err
}
