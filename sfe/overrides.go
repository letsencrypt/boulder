...
func setOverrideRequestFormHeaders(w http.ResponseWriter) {
	...
	w.Header().Set("Content-Security-Policy", strings.Join([]string{
		// Only allow content from the same origin and HTTPS.
		"default-src 'self' https:",
		// Only allow only same-origin scripts.
		"script-src 'self'",
		// Only allow same-origin styles.
		"style-src 'self'",
		// Disallow <object>, <embed>, <applet>, etc.
		"object-src 'none'",
...
func (sfe *SelfServiceFrontEndImpl) NewOrderPerAccountOverrideRequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	sfe.renderTemplate(w, overrideFormTemplate, map[string]any{
		"FormHTML":          newOrdersPerAccountForm + fundraisingField.RenderField(),
...
func (sfe *SelfServiceFrontEndImpl) CertificatesPerDomainOverrideRequestHandler(w http.ResponseWriter, r *http.Request) {
	setOverrideRequestFormHeaders(w)
	sfe.renderTemplate(w, overrideFormTemplate, map[string]any{
		"FormHTML":          certificatesPerDomainForm + fundraisingField.RenderField(),
...
func (sfe *SelfServiceFrontEndImpl) CertificatesPerIPOverrideRequestHandler(w http.ResponseWriter, r *http.Request) {
	setOverrideRequestFormHeaders(w)
	sfe.renderTemplate(w, overrideFormTemplate, map[string]any{
		"FormHTML":          certificatesPerIPForm + fundraisingField.RenderField(),
		"RateLimit":         "CertificatesPerDomain",
		"ValidateFieldPath": overridesValidateField,
		"SubmitRequestPath": overridesSubmitRequest,
		"SubmitSuccessPath": overridesSubmitSuccess,
...
func (sfe *SelfServiceFrontEndImpl) CertificatesPerDomainPerAccountOverrideRequestHandler(w http.ResponseWriter, r *http.Request) {
	setOverrideRequestFormHeaders(w)
	sfe.renderTemplate(w, overrideFormTemplate, map[string]any{
		"FormHTML":          certificatesPerDomainPerAccountForm + fundraisingField.RenderField(),
...
// rate limit. If the request is successful, it redirects to the success page;
...
func (sfe *SelfServiceFrontEndImpl) SubmitOverrideRequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, name := range []string{
		// Note that not all of these fields will be included in the Zendesk
		// ticket, but they are required for the submission to be considered
		// valid.
		rlo.SubscriberAgreementFieldName,
		rlo.PrivacyPolicyFieldName,
		rlo.MailingListFieldName,
