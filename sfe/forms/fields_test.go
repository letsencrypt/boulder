...
func TestInputField_RenderField(t *testing.T) {
	cases := []struct {
		name     string
		required bool
	}{
...
func TestDropdownField_RenderField(t *testing.T) {
	cases := []struct {
		name     string
		required bool
	}{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Ensure constructor returns the intended concrete type via the interface.
			df, ok := f.(*DropdownField)
			if !ok {
				t.Fatalf("NewDropdownField did not return *DropdownField")
			}
			if df == nil {
				t.Fatalf("nil *DropdownField")
			}
...
func TestTextareaField_RenderField(t *testing.T) {
	cases := []struct {
		name     string
		rows     int
		required bool
...
func TestCheckboxField_RenderField(t *testing.T) {
	cases := []struct {
		name     string
		required bool
	}{
