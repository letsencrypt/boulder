...
var invalidCSRs = []string{
	// Bad signatures
	// MD5WithRSA
	"",
	// SHA1WithRSA
...
func TestVerifyCSR(t *testing.T) {
	for _, csrHex := range validCSRs {
		csrDER, _ := hex.DecodeString(csrHex)
		csr, _ := x509.ParseCertificateRequest(csrDER)
		err := VerifyCSR(csr)
		if err != nil {
			t.Errorf("Error verifying valid CSR: %v", err)
		}
	}

	for _, csrHex := range invalidCSRs {
		csrDER, _ := hex.DecodeString(csrHex)
		csr, _ := x509.ParseCertificateRequest(csrDER)
		err := VerifyCSR(csr)
		if err == nil {
			t.Errorf("Invalid CSR did not return an error")
		}
	}
}
