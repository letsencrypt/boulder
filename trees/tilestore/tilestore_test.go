...
func (b backendTiles) Height() int {
	return tile.Height
}
...
// countingBackend wraps a Backend and counts Get calls.
...
func TestCorruptionNeverYieldsValidProof(t *testing.T) {
	...
	const start, end = 256, 264 // a complete size-8 subtree in the ragged right edge
	// A complete subtree has a single stored hash, at level 3, index start>>3,
	// which the independent reference reader supplies.
	ref, _ := reference(t, all)
	trueNodes, err := ref.ReadHashes([]int64{tlog.StoredHashIndex(3, start>>3)})
	if err != nil {
		t.Fatalf("ReadHashes: %s", err)
	}
	trueNode := trueNodes[0]
...
func TestReadLeaves(t *testing.T) {
	...
	leaves, err := s.ReadLeaves(t.Context(), start, end, tree.N)
	if err != nil {
...
func TestReadEntries(t *testing.T) {
	...
	got, err := s.ReadEntries(t.Context(), 250, 260, 300)
	if err != nil {
	...
	full, err := s.ReadEntries(t.Context(), 0, 300, 300)
	if err != nil {
...
func TestRecovery(t *testing.T) {
	...
	emptyHash := tlog.Hash(sha256.Sum256(nil))
	if empty.N != 0 || empty.Hash != emptyHash {
		t.Errorf("empty tree = {%d, %x}, want {0, %x}", empty.N, empty.Hash, emptyHash)
	}
...
// TestHashReaderCachesVerifiedTiles confirms a HashReader serves repeat reads
// from the tiles it has already verified rather than re-fetching them from the
// Backend, and that the cached reads still reconstruct the correct root.
func TestHashReaderCachesVerifiedTiles(t *testing.T) {
	counting := &countingBackend{inner: fs.New(t.TempDir())}
	s := tilestore.New(counting, testOrigin)
	tree, err := s.Append(t.Context(), 0, entries(0, 300))
	if err != nil {
		t.Fatalf("Append: %s", err)
	}

	reader := s.HashReader(t.Context(), tree)
	root, err := tlog.TreeHash(tree.N, reader)
	if err != nil {
		t.Fatalf("first TreeHash: %s", err)
	}
	if root != tree.Hash {
		t.Fatalf("first root = %x, want %x", root, tree.Hash)
	}

	warm := counting.gets
	root, err = tlog.TreeHash(tree.N, reader)
	if err != nil {
		t.Fatalf("second TreeHash: %s", err)
	}
	if root != tree.Hash {
		t.Fatalf("second root = %x, want %x", root, tree.Hash)
	}
	if counting.gets != warm {
		t.Errorf("second TreeHash did %d backend Gets, want 0", counting.gets-warm)
	}
}
...
func TestSubtreeProofOverStore(t *testing.T) {
	...
	// [128, 256) is a complete subtree with a single stored hash at level 7,
	// index 1, which the independent reference reader supplies.
	ref, _ := reference(t, all)
	nodes, err := ref.ReadHashes([]int64{tlog.StoredHashIndex(7, 1)})
	if err != nil {
		t.Fatalf("ReadHashes: %s", err)
	}
	if !subtree.VerifyConsistency(start, end, tree.N, proof, nodes[0], tree.Hash) {
