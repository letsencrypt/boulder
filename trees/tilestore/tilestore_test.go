...
func TestCorruptionNeverYieldsValidProof(t *testing.T) {
	...
	trueNode := subtree.HashLeaves(trueLeaves)
...
func TestRecovery(t *testing.T) {
	...
	if empty.N != 0 || empty.Hash != subtree.HashLeaves(nil) {
		t.Errorf("empty tree = {%d, %x}, want {0, %x}", empty.N, empty.Hash, subtree.HashLeaves(nil))
	}
...
func TestSubtreeProofOverStore(t *testing.T) {
	...
	nodeHash := subtree.HashLeaves(leaves)
