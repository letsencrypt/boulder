...
//
// A Store manages one log and is single-writer: only one process may append to
// a given log. Reads may run concurrently with each other.
//
...
// ErrNotExist is the error, possibly wrapped, that a Backend's Get returns when
// no object has the requested key.
...
// Put overwrites. Get returns an error matching ErrNotExist for an absent key.
// Implementations must be safe for concurrent use. Batching and parallelism
// belong above the interface or inside an implementation.
...
type Store struct {
	...
	// appendMu serializes Append within this process. It does not make the log
	// multi-writer: a second process appending to the same log can still
	// corrupt it.
	appendMu sync.Mutex
}
...
// stateKey names the object holding the authoritative tree size and root. It is
// not part of the tlog-tiles layout, and Fetch does not serve it.
...
// Tree returns the log's current size and root hash. An empty log returns size
// zero and the hash of the empty string.
func (s *Store) Tree(ctx context.Context) (tlog.Tree, error) {
	b, err := s.backend.Get(ctx, s.key(stateKey))
	if errors.Is(err, ErrNotExist) {
		// RFC 9162: the hash of an empty tree is the hash of the empty string.
		return tlog.Tree{Hash: tlog.Hash(sha256.Sum256(nil))}, nil
	}
	if err != nil {
	...
	var tree tlog.Tree
	tree.N = int64(binary.BigEndian.Uint64(b[:8])) //nolint:gosec // G115: written by Append from a non-negative size.
	copy(tree.Hash[:], b[8:])
	return tree, nil
}
...
// stale: tile objects are immutable per path, because a grown partial tile gets
// a new width-qualified path.
...
// verified checkpoint or from Tree. The reader caches the tiles it has verified
// for its lifetime, so reuse one reader across related reads.
...
// size treeSize. treeSize must be a size this log reached through Append, such
// as the current size from Tree or the size of a checkpoint over this log: the
// bundle at a tree's right edge is stored at a width keyed to that size, and a
// read at any other size misses it.
...
// ReadCheckpoint returns the stored checkpoint bytes, or nil with no error when
// the log has no checkpoint yet.
...
// extendReader returns a HashReader for the transitional tree an Append builds,
// part stored and part new. Stored-hash indices below oldCount come from old,
// the verifying reader over the already-written tiles. Index oldCount+k is
// (*added)[k], a hash computed earlier in this Append. added is a pointer so
// the reader sees the slice grow as Append fills it.
...
// oldSize+len(entries)). When the first bundle already holds entries below
// oldSize, they are read back and kept, so the grown bundle stays complete. A
// superseded partial bundle stays in place under its old width-qualified key: a
// Backend cannot delete, and those objects are what keeps ReadEntries at an
// earlier tree size working.
...
// Entries and tiles are written before the tree state, so a failed Append never
// advances the tree, and retrying at the same size is safe.
func (s *Store) Append(ctx context.Context, atSize int64, entries [][]byte) (tlog.Tree, error) {
	...
	state := make([]byte, 8+tlog.HashSize)
	binary.BigEndian.PutUint64(state[:8], uint64(tree.N)) //nolint:gosec // G115: tree size is non-negative.
	copy(state[8:], tree.Hash[:])
	err = s.backend.Put(ctx, s.key(stateKey), state)
	if err != nil {
		return tlog.Tree{}, err
	}
	return tree, nil
