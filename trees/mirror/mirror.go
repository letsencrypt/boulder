// Package mirror encodes and decodes the c2sp.org/tlog-mirror request and
// response bodies. It does no I/O and holds no state: a submitter marshals
// the requests a mirror parses, and the mirror marshals the responses the
// submitter parses.
//
// An add-entries upload is organized as a canonical sequence of entry
// packages, each aligned to a 256-entry tile and independently authenticated
// by a subtree consistency proof. EntryPackages and EntryPackageAt compute
// the package boundaries, and both sides derive the same sequence from the
// upload interval alone.
//
...
func EntryPackageAt(uploadStart, uploadEnd, i int64) (Package, bool) {
	...
	// Written as a subtraction: base+tile.Width could overflow int64.
...
// Marshal encodes the request for the wire, checking Packages against the
// canonical sequence.
func (r AddEntriesRequest) Marshal() ([]byte, error) {
	...
	// Presize the body. A chunked upload's body reaches tens of megabytes,
	// which append growth would copy repeatedly.
	size := 2 + len(r.Origin) + 8 + 8 + 2 + len(r.Ticket)
	for _, p := range r.Packages {
		size += 1 + len(p.Proof)*tlog.HashSize
		for _, e := range p.Entries {
			size += 2 + len(e)
		}
	}

	out := binary.BigEndian.AppendUint16(make([]byte, 0, size), uint16(len(r.Origin))) //nolint:gosec // G115: len(r.Origin) is bounded by the math.MaxUint16 check above.
...
//
// truncated reports that the body ended partway through a package: req then
// holds the complete packages before the cut, which the mirror processes as a
// prefix upload. Reject an empty-but-truncated request (truncated set with no
// complete package). Violations that cannot come from an interrupted body,
// such as more packages than the canonical sequence holds, are errors
// instead.
