...
func TestTilePath(t *testing.T) {
	...
	// tlog.ParseTilePath re-marshals the parsed tile and rejects a mismatch, so
	// non-canonical spellings of valid coordinates must fail.
	t.Run("Rejects non-canonical paths", func(t *testing.T) {
		for _, path := range []string{
			"tile/01/000",     // zero-padded level
			"tile/0/00",       // two-digit index group
			"tile/0/1234",     // four-digit index group
			"tile/0/001/234",  // missing x prefix
			"tile/0/x000/123", // leading zero group
			"tile/0/000.p/07", // zero-padded width
		} {
			_, err := ParsePath(path)
			if err == nil {
				t.Errorf("ParsePath(%q) = nil error, want error", path)
...
func TestBundleRange(t *testing.T) {
	cases := []struct {
		...
		{"Single entry", 0, 1, 0, 0},
		{"Within one bundle", 5, 10, 0, 0},
		{"Exactly one full bundle", 0, Width, 0, 0},
		{"Spans into the second bundle", 0, Width + 1, 0, 1},
		{"Aligned second bundle", Width, 2 * Width, 1, 1},
		{"Straddles a boundary", Width - 1, Width + 1, 0, 1},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			first, last := BundleRange(tc.start, tc.end)
			if first != tc.expectFirst || last != tc.expectLast {
				t.Errorf("BundleRange(%d, %d) = (%d, %d), want (%d, %d)", tc.start, tc.end, first, last, tc.expectFirst, tc.expectLast)
			}
		})
	}
}
