...
// Package cosignature signs and verifies ML-DSA-44 checkpoint cosignatures, the
// c2sp.org/tlog-cosignature scheme the MTC draft adopts for mirrors.
//
// What gets signed: the cosigner is handed a checkpoint note text. It parses
// out the origin, tree size, and root hash, rebuilds them as a cosigned.Message
// (the draft's cosigned_message struct, labeled "subtree/v1", carrying the
// cosigner's own name, the signing time, start 0, and end set to the tree
// size), and signs that message with ML-DSA-44. Checkpoint extension lines are
// not covered. The wire signature is a timestamped_signature, the 8-byte
// big-endian signing time followed by the 2420-byte ML-DSA-44 signature,
// because a verifier needs the time to rebuild the same message.
//
// The types plug into golang.org/x/mod/sumdb/note: MLDSACosigner is a
// note.Signer and MLDSACosignatureVerifier is a note.Verifier, so cosigning and
// verification ride the standard signed-note flows. Cosignature and
// CosignatureLine convert between the note signature line and the raw
// timestamped_signature a database stores.
//
...
// cosignatureAlg is the tlog-cosignature algorithm byte for ML-DSA-44, one of
// the inputs to the key ID computation.
const cosignatureAlg = 0x06

// CosignatureKeyID returns the tlog-cosignature key ID of an ML-DSA-44
// cosigner, SHA-256(name || "\n" || 0x06 || pubkey)[:4] as a big-endian uint32,
// the value this package's signer and verifier report as their note KeyHash.
// Four bytes is not collision resistant, so it routes signatures to keys rather
...
// marshalCosignedMessage serializes the whole-tree cosigned.Message over [0,
// end). Name and origin length limits are checked by cosigned.Message.Marshal.
...
// checkpoints. It implements note.Signer for use with note.Sign, and Cosign
// covers the direct path. It is read-only after construction, so one cosigner
// may be shared.
...
// isValidCosignerName reports whether name can be a signed-note key name:
// non-empty UTF-8 with no Unicode spaces or plus signs (mirroring x/mod's
...
// hand rather than through note.Sign, so this check is the only gate.
...
// Sign cosigns the checkpoint in text and returns the raw
// timestamped_signature: the 8-byte big-endian signing time followed by the
// ML-DSA-44 signature. It refuses a text that is not a checkpoint, since a
// cosignature attests to a tree head and nothing else. Sign is the note.Signer
// entry point that note.Sign calls. Use Cosign when you want the wire signature
// line instead of the bare bytes.
//
//   - https://c2sp.org/tlog-cosignature
//   - https://ietf-plants-wg.github.io/merkle-tree-certs/draft-ietf-plants-merkle-tree-certs.html#section-5.3.1
func (c *MLDSACosigner) Sign(text []byte) ([]byte, error) {
	parsed, err := checkpoint.Unmarshal(string(text))
	if err != nil {
		return nil, fmt.Errorf("refusing to cosign non-checkpoint message: %w", err)
	}
	// Unmarshal canonicalizes every field it accepts and Marshal cannot error
	// on a checkpoint Unmarshal produced, so this equality cannot fail today.
	// It guards the signed message against Unmarshal ever growing more lenient.
	canonical, err := parsed.Marshal()
	if err != nil {
		return nil, fmt.Errorf("refusing to cosign: %w", err)
	}
...
// Cosign cosigns the checkpoint in text and returns the complete signed-note
...
func (c *MLDSACosigner) Cosign(text []byte) (string, error) {
	sig, err := c.Sign(text)
	if err != nil {
		return "", err
	}
...
// CosignatureLine rebuilds the signed-note signature line for a stored raw
// timestamped_signature, recomputing the key ID from name and pub so no private
// key is needed. It does not verify the signature, so the blob must come from a
// trusted source, such as one Cosignature returned from an opened note.
...
// Cosignature extracts v's raw timestamped_signature from n, the bytes to
// persist or to feed back to CosignatureLine. It returns false when v did not
// sign n or the signature is not a well-formed timestamped_signature. n must
// come from note.Open with v among the verifiers, since only then does n.Sigs
// hold verified signatures.
...
// implements note.Verifier: pass it to note.Open, then hand the opened note to
// Cosignature to extract the raw signature.
...
// over the checkpoint in text. It is the note.Verifier entry point that
// note.Open calls.
//
//   - https://c2sp.org/tlog-cosignature
//   - https://ietf-plants-wg.github.io/merkle-tree-certs/draft-ietf-plants-merkle-tree-certs.html#section-5.3.1
func (v *MLDSACosignatureVerifier) Verify(text, sig []byte) bool {
