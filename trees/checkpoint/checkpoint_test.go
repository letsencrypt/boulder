...
func TestCheckpointUnmarshalRejects(t *testing.T) {
	cases := []struct {
		...
		// note.Open enforces that on whole notes, but
		// cosignature.MLDSACosigner.Sign hands Unmarshal raw checkpoint text
		// that never passes through note.Open, so Unmarshal must enforce it
		// itself.
