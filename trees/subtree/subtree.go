// Package subtree implements the MTC draft's subtree consistency proofs over
// the RFC 9162 Merkle tree: HashLeaves computes MTH(D[start:end]),
// ConsistencyProof shows that a subtree sits at a given position in a larger
// tree, and VerifyConsistency checks such a proof.
//
// To read this package against the draft (sections 4.1 and 4.4, appendices
// B.4 and B.5), the correspondence is:
//
//   - ConsistencyProof is SUBTREE_PROOF, and subtreeSubProof is
//     SUBTREE_SUBPROOF with the draft's b flag named known.
//   - splitPoint is the draft's mid.
//   - valid is the section 4.1 subtree definition, including the BIT_CEIL
//     alignment condition.
//   - VerifyConsistency follows appendix B.5 and keeps the draft's fn, sn,
//     and tn variable names.
//
// One deliberate divergence: the draft's pseudocode rebases coordinates at
// each recursion, slicing D[k:n] so ranges restart at zero. This package
// keeps absolute leaf positions throughout and instead passes the enclosing
// node down as a [windowStart, windowEnd) window. The tlog.HashReader backing
// the proofs is indexed absolutely, so rebasing would put offset arithmetic
// at every read.
//
// x/mod/sumdb/tlog keeps every node hash of the tree in one flat stored-hash
// index space, and only complete subtrees (power-of-two sized and aligned)
// have a stored hash. hashSubtree therefore decomposes a range into its
// complete subtrees, reads their stored hashes in one batched ReadHashes
// call, and recombines them. completeSubtreeIndexes and combineSubtreeRoots
// must walk that decomposition in the same order: that contract is the
// remainder slice the latter threads through.
package subtree
