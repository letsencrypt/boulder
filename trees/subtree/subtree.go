...
func Hash(leaves []tlog.Hash) tlog.Hash {
	...
	// Split into two subtree roots, the left a perfect subtree and the right
	// the remainder, then hash them as SHA-256(0x01 || left || right).
...
// section 4.1 Definition of a Subtree.
...
func rangeHash(lo, hi int64, reader tlog.HashReader) (tlog.Hash, error) {
	...
	if len(hashes) != len(indexes) {
		// foldRangeHash indexes hashes positionally and would panic on a short
		// slice, so reject a mismatched count here.
...
func subtreeSubProof(start, end, base, n int64, known bool, reader tlog.HashReader, proof []tlog.Hash) ([]tlog.Hash, error) {
	if start == 0 && end == n {
		// Base case: [start, end) covers this whole node D_n. known means the
		// verifier already has it, so emit nothing. Otherwise emit MTH(D_n).
		if known {
			return proof, nil
		}
	...
	default:
		...
		// subtree only does when start == 0. The right child's prefix [0,
		// end-k) is not a whole node the verifier holds, so known becomes
		// false. Left child [0, k) is the sibling.
