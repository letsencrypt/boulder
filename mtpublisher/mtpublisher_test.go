...
// synthSourceLog writes n entries and a log-signed checkpoint into an on-disk
// tilestore at dir, the way the publisher reads its source log. It returns the
// resulting tree and the log's note verifiers.
func synthSourceLog(t *testing.T, dir string, n int) (tlog.Tree, note.Verifiers) {
	t.Helper()
	...
	src := tilestore.New(fs.New(dir), testOrigin)
	entries := make([][]byte, n)
	for i := range entries {
	...
	return tree, note.VerifierList(verifier)
}
...
func (m *thinMirror) handler() http.Handler {
	...
	mux.HandleFunc("POST /add-checkpoint", func(w http.ResponseWriter, r *http.Request) {
		...
		cp, n, err := checkpoint.Verify(req.Checkpoint, m.verifiers)
		if err != nil {
...
// use, the source tree, and the mirror's public key for re-validation.
func startMirror(t *testing.T) (MirrorConfig, tlog.Tree, *mldsa.PublicKey) {
	t.Helper()
	srcDir := t.TempDir()
	tree, logVerifiers := synthSourceLog(t, srcDir, 300)
...
func TestMirrorLog(t *testing.T) {
	srcDir := t.TempDir()
	tree, logVerifiers := synthSourceLog(t, srcDir, 300)
	src := tilestore.New(fs.New(srcDir), testOrigin)
	...
	cosig, err := mirrorLog(t.Context(), srv.Client(), srv.URL, testOrigin, src, verifier)
	if err != nil {
...
func TestPublish(t *testing.T) {
	dbMap := setupDB(t)
	mc, tree, pub := startMirror(t)
	p, err := New(dbMap, time.Second, mtcLogID, mirrorID, mc, clock.NewFake(), blog.NewMock())
	if err != nil {
		t.Fatalf("New: %s", err)
	}
...
func TestPublishWhenLatestAlreadySigned(t *testing.T) {
	dbMap := setupDB(t)
	p, err := New(dbMap, time.Second, mtcLogID, mirrorID, validMirrorConfig(t), clock.NewFake(), blog.NewMock())
	if err != nil {
		t.Fatalf("New: %s", err)
	}
