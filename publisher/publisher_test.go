...
func TestProbeLogs(t *testing.T) {
	...
	test.AssertNotError(t, err, "x509.MarshalPKIXPublicKey(der1) failed")
	k1b64 := base64.StdEncoding.EncodeToString(der1)
	_, err = pub.ctLogsCache.AddLog(fmt.Sprintf("http://localhost:%d", portA), k1b64, pub.log)
	...
	test.AssertNotError(t, err, "x509.MarshalPKIXPublicKey(der2) failed")
	k2b64 := base64.StdEncoding.EncodeToString(der2)
	_, err = pub.ctLogsCache.AddLog(fmt.Sprintf("http://localhost:%d", portB), k2b64, pub.log)
	test.AssertNotError(t, err, "Failed to add log to logCache")
	k3, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	test.AssertNotError(t, err, "ecdsa.GenerateKey() failed for k3")
	der3, err := x509.MarshalPKIXPublicKey(&k3.PublicKey)
	test.AssertNotError(t, err, "x509.MarshalPKIXPublicKey(der3) failed")
	k3b64 := base64.StdEncoding.EncodeToString(der3)
	_, err = pub.ctLogsCache.AddLog("http://blackhole:9999", k3b64, pub.log)
	test.AssertNotError(t, err, "Failed to add log to logCache")
