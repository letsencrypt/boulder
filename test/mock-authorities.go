...
func InitAuthorities(t *testing.T) (*blog.AuditLogger, core.CertificateAuthority, *DummyValidationAuthority, *sa.SQLStorageAuthority, core.RegistrationAuthority) {
	...
	ra := ra.NewRegistrationAuthorityImpl(audit)
	ra.SA = sa
	ra.VA = va
	ra.CA = &ca
	ra.PA = pa

	return audit, &ca, va, sa, &ra
}
