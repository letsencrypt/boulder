...
func main() {
	...
	log.Printf("OAuth server running on port %d", c.OAuthPort)
	go cmd.FailOnError(http.ListenAndServe(fmt.Sprintf(":%d", c.OAuthPort), oauthMux), "Failed to start OAuth server")
	...
	log.Printf("pardot-test-srv running on port %d", c.PardotPort)
	cmd.FailOnError(http.ListenAndServe(fmt.Sprintf(":%d", c.PardotPort), pardotMux), "Failed to start Pardot server")
}
