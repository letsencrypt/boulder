...
func TestCAALogChecker(t *testing.T) {
	...
	cmd := exec.Command("bin/boulder", "caa-log-checker", "-ra-log", "/var/log/ra.log", "-va-logs", "/var/log/va.log")
	...
	cmd = exec.Command("bin/boulder", "caa-log-checker", "-ra-log", "/var/log/ra.log", "-va-logs", tmp.Name())
	stdErr = new(bytes.Buffer)
	cmd.Stderr = stdErr
	out, err = cmd.Output()
