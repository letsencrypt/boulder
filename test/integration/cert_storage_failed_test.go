...
func TestIssuanceCertStorageFailed(t *testing.T) {
	...
	_, err = authAndIssue(nil, certKey, []identifier.ACMEIdentifier{identifier.NewDNS(revokeMeDomain)}, true)
	...
	// This should fail because the trigger prevented setting the certificate status to "ready"
	_, err = authAndIssue(nil, certKey, []identifier.ACMEIdentifier{identifier.NewDNS(blockMyKeyDomain)}, true)
	test.AssertError(t, err, "expected authAndIssue to fail")
	...
	res, err := authAndIssue(nil, certKey, []identifier.ACMEIdentifier{identifier.NewDNS(random_domain())}, true)
	...
	_, err = authAndIssue(nil, certKey, []identifier.ACMEIdentifier{identifier.NewDNS("123.example.com")}, true)
	test.AssertError(t, err, "expected authAndIssue to fail")
