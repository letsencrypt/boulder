...
func TestCRLPipeline(t *testing.T) {
	...
	res, err := authAndIssue(client, nil, []identifier.ACMEIdentifier{identifier.NewDNS(random_domain())}, true)
...
func TestTemporalAndExplicitShardingCoexist(t *testing.T) {
	...
	// in both worlds.
	result, err := authAndIssue(client, certKey, []identifier.ACMEIdentifier{identifier.NewDNS(random_domain())}, true)
	if err != nil {
