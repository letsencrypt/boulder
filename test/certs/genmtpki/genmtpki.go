...
func main2() error {
	outputDir := flag.String("output-dir", "/tmp/", "Directory to write outputs to")
	flag.Parse()
	...
	keyFile, err := os.OpenFile(*outputDir+basename+".key.pem", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
	if err != nil {
		return err
	}
	defer keyFile.Close()
	...
	certFile, err := os.OpenFile(*outputDir+basename+".cert.pem", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
	if err != nil {
		return err
	}
...
func mtcaExtension() (pkix.Extension, error) {
	...
	oidSignatureMLDSA44 := asn1.ObjectIdentifier{1, 2, 840, 1, 101, 3, 4, 3, 17}
