...
with boulder-mtca within a running boulder container.
...
To list all tiles (while bminio is running):
    ...
    alias mc="cd ~/boulder && docker compose exec bminio mc"
...
To clear tile storage (note: also run clear.sh to clear the DB):

    mc rm -r --force local/boulder-mtc-tiles/

Alternatively, to reset all storage (DB and tiles), you can always run,
