...
def test_http_challenge_loop_redirect():
    ...
    # redirect loop.
    chisel2.expect_problem("urn:ietf:params:acme:error:connection", lambda: chisel2.auth_and_issue([d], client=client, chall_type="http-01"))
...
def test_http_challenge_badport_redirect():
    ...
    # an invalid port.
    challengePath = "/.well-known/acme-challenge/{0}".format(token)
    challSrv.add_http_redirect(challengePath, "http://{0}:1337{1}".format(d, challengePath))
...
def test_http_challenge_badhost_redirect():
    ...
    # hostname.
    challengePath = "/.well-known/acme-challenge/{0}".format(token)
    challSrv.add_http_redirect(challengePath, "https://127.0.0.1{0}".format(challengePath))
...
def test_http_challenge_badproto_redirect():
    ...
    challengePath = "/.well-known/acme-challenge/{0}".format(token)
    challSrv.add_http_redirect(challengePath, "gopher://{0}{1}".format(d, challengePath))
...
def test_http_challenge_timeout():
    ...
    try:
        ...
        chisel2.expect_problem("urn:ietf:params:acme:error:connection", lambda: chisel2.auth_and_issue([hostname], chall_type="http-01"))
...
def test_overlapping_wildcard():
    ...
    if len(authzs) != 2:
        raise (Exception("order for %s had %d authorizations, expected 2" % (domains, len(authzs))))
...
def test_highrisk_blocklist():
    ...
    chisel2.expect_problem("urn:ietf:params:acme:error:rejectedIdentifier", lambda: chisel2.auth_and_issue([domain], chall_type="dns-01"))
...
def test_wildcard_authz_reuse():
    ...
    for authz in order.authorizations:
        if authz.body.status != Status("pending"):
            raise (Exception("order for %s included a non-pending authorization (status: %s) from a previous HTTP-01 order" % ((domains), str(authz.body.status))))


def test_bad_overlap_wildcard():
    chisel2.expect_problem("urn:ietf:params:acme:error:malformed", lambda: chisel2.auth_and_issue(["*.example.com", "www.example.com"]))
...
def test_order_reuse_failed_authz():
    ...
    for authz in order.authorizations:
        if authz.body.status != Status("pending"):
            raise (Exception("order for %s included a non-pending authorization (status: %s) from a previous order" % ((domains), str(authz.body.status))))
...
def test_order_finalize_early():
    ...
    chisel2.expect_problem("urn:ietf:params:acme:error:orderNotReady", lambda: client.finalize_order(order, deadline))
...
def test_revoke_by_privkey():
    ...
    cert_file = tempfile.NamedTemporaryFile(dir=tempdir, suffix=".test_revoke_by_privkey.pem", mode="w+", delete=False)
...
def test_double_revocation():
    ...
    cert_file = tempfile.NamedTemporaryFile(dir=tempdir, suffix=".test_double_revoke.pem", mode="w+", delete=False)
    cert_file.write(OpenSSL.crypto.dump_certificate(OpenSSL.crypto.FILETYPE_PEM, cert).decode())
    cert_file.close()
...
def test_only_return_existing_reg():
    ...
    client.new_account(messages.NewRegistration.from_data(email=email, terms_of_service_agreed=True))
    ...
    acct = extendedAcct({"termsOfServiceAgreed": True, "contact": [email], "onlyReturnExisting": True})
    ...
    newAcct = extendedAcct({"termsOfServiceAgreed": True, "contact": [email], "onlyReturnExisting": True})
    chisel2.expect_problem("urn:ietf:params:acme:error:accountDoesNotExist", lambda: other_client.net.post(other_client.directory["newAccount"], newAcct))
...
def test_delete_unused_challenges():
    ...
    for _ in range(5):
        ...
        time.sleep(1)
    if len(a.body.challenges) != 1:
        raise (Exception("too many challenges (%d) left after failed validation" % len(a.body.challenges)))
    if not isinstance(a.body.challenges[0].chall, challenges.DNS01):
        raise (Exception("wrong challenge type left after validation"))
...
def test_recheck_caa():
    ...
    for a in caa_recheck_setup_data["authzs"]:
        ...
        if response.status_code != 200:
            raise (Exception("Unexpected response for CAA authz: ", response.status_code))
    ...
    chisel2.expect_problem("urn:ietf:params:acme:error:caa", lambda: chisel2.auth_and_issue(domains, client=caa_recheck_setup_data["client"]))
...
def test_caa_reject():
    ...
    chisel2.expect_problem("urn:ietf:params:acme:error:caa", lambda: chisel2.auth_and_issue([domain]))
...
def test_caa_extensions():
    ...
    chisel2.expect_problem("urn:ietf:params:acme:error:caa", lambda: chisel2.auth_and_issue(["dns-01-only.good-caa-reserved.com"], chall_type="http-01"))

    chisel2.expect_problem("urn:ietf:params:acme:error:caa", lambda: chisel2.auth_and_issue(["http-01-only.good-caa-reserved.com"], chall_type="dns-01"))
...
def test_renewal_exemption():
    ...
    chisel2.expect_problem("urn:ietf:params:acme:error:rateLimited", lambda: chisel2.auth_and_issue(["mail." + base_domain]))


def test_certificates_per_name():
    chisel2.expect_problem("urn:ietf:params:acme:error:rateLimited", lambda: chisel2.auth_and_issue([random_domain() + ".lim.it"]))
...
def test_oversized_csr():
    ...
    chisel2.expect_problem("urn:ietf:params:acme:error:malformed", lambda: chisel2.auth_and_issue(domains))
...
def test_admin_revoker_cert():
    ...
    reset_akamai_purges()
    run(["./bin/boulder", "admin-revoker", "serial-revoke", "--config", "%s/admin-revoker.json" % config_dir, "%x" % parsed_cert.serial_number, "1"])
...
def test_admin_revoker_batched():
    serialFile = tempfile.NamedTemporaryFile(dir=tempdir, suffix=".test_admin_revoker_batched.serials.hex", mode="w+", delete=False)
    ...
    run(["./bin/boulder", "admin-revoker", "batched-serial-revoke", "--config", "%s/admin-revoker.json" % config_dir, serialFile.name, "0", "2"])
...
def test_sct_embedding():
    ...
    for sct in sctList.value:
        ...
        if abs(delta) > datetime.timedelta(hours=1):
            raise (Exception("Delta between SCT timestamp and now was too great " "%s vs %s (%s)" % (sct.timestamp, datetime.datetime.now(), delta)))
...
def ocsp_resigning_setup():
    ...
    ocsp_response, reason = get_ocsp_response_and_reason(cert_file.name, "/hierarchy/intermediate-cert-rsa-a.pem", "http://localhost:4002")
    global ocsp_resigning_setup_data
    ocsp_resigning_setup_data = {"cert_file": cert_file.name, "response": ocsp_response, "reason": reason}
...
def test_ocsp_resigning():
    ...
    tries = 0
    while tries < 5:
        resp, reason = get_ocsp_response_and_reason(ocsp_resigning_setup_data["cert_file"], "/hierarchy/intermediate-cert-rsa-a.pem", "http://localhost:4002")
    ...
    if reason != ocsp_resigning_setup_data["reason"]:
        raise (Exception("re-signed ocsp response has different reason %s expected %s" % (reason, ocsp_resigning_setup_data["reason"])))
