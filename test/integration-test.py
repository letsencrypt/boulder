...
def test_single_ocsp():
    ...
    """
    p = subprocess.Popen(["./bin/boulder", "ocsp-responder", "--config", "test/issuer-ocsp-responder.json"])
...
def main():
    ...
    parser.add_argument("--chisel", dest="run_chisel", action="store_true", help="run integration tests using chisel")
    parser.add_argument("--gotest", dest="run_go", action="store_true", help="run Go integration tests")
    parser.add_argument("--filter", dest="test_case_filter", action="store", help="Regex filter for test cases")
...
def check_slow_queries():
    ...
    query = """
        ...
        \G
    """
    output = subprocess.check_output(["mysql", "-h", "boulder-proxysql", "-e", query], stderr=subprocess.STDOUT).decode()
    if len(output) > 0:
        print(output)
...
def run_loadtest():
    ...
    run(["./bin/load-generator", "-config", "test/load-generator/config/integration-test-config.json", "-results", latency_data_file])
