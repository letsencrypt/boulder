...
def temppath(name):
    ...
    f = tempfile.NamedTemporaryFile(dir=tempdir, suffix=".{0}".format(name), mode="w+", delete=False)
    f.close()
    return f
...
def get_future_output(cmd, date):
    return subprocess.check_output(cmd, stderr=subprocess.STDOUT, env={"FAKECLOCK": fakeclock(date)}).decode()
...
def make_ocsp_req(cert_file, issuer_file):
    ...
    with tempfile.NamedTemporaryFile(dir=tempdir) as f:
        run(["openssl", "ocsp", "-no_nonce", "-issuer", issuer_file, "-cert", cert_file, "-reqout", f.name])
        ocsp_req = f.read()
    return ocsp_req
...
def ocsp_verify(cert_file, issuer_file, ocsp_response):
    with tempfile.NamedTemporaryFile(dir=tempdir, delete=False) as f:
        ...
        output = subprocess.check_output(
            ["openssl", "ocsp", "-no_nonce", "-issuer", issuer_file, "-cert", cert_file, "-verify_other", issuer_file, "-CAfile", "/hierarchy/root-cert-rsa.pem", "-respin", f.name],
...
def waithealth(prog, addr):
    subprocess.check_call(["./bin/health-checker", "-addr", addr, "-config", os.path.join(config_dir, "health-checker.json")])
