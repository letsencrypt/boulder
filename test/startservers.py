...
SERVICES = (
    ...
    Service('nonce-service-taro',
        ...
        ('./bin/boulder', 'nonce-service', '--config', os.path.join(config_dir, 'nonce.json'), '--addr', '10.77.77.77:9101', '--debug-addr', ':8111', '--prefix', 'taro'),
        None),
    Service('nonce-service-zinc',
        8112, 'nonce2.service.consul:9101',
        ('./bin/boulder', 'nonce-service', '--config', os.path.join(config_dir, 'nonce.json'), '--addr', '10.88.88.88:9101', '--debug-addr', ':8112', '--prefix', 'zinc'),
        None),
