...
func (s *server) handleAddCheckpoint(w http.ResponseWriter, r *http.Request) {
	...
	cp, n, err := checkpoint.Verify(req.Checkpoint, ls.verifiers)
	if err != nil {
...
func (s *server) handleAddEntries(w http.ResponseWriter, r *http.Request) {
	...
	// A ragged first package proves a subtree whose leading entries the mirror
	// already holds; read that prefix of leaves from the store.
	prefixStart := req.UploadStart - req.UploadStart%tile.Width
	prefix, err := ls.store.ReadLeaves(ctx, tree.N, prefixStart, req.UploadStart)
	if err != nil {
	...
	for i, pkg := range req.Packages {
		...
		subtreeHash := subtree.Hash(leaves)
