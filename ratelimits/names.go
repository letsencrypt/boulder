...
func validateDomain(id string) error {
	...
	if err != nil {
		return fmt.Errorf("invalid domain, %q must be formatted 'domain': %s", id, err)
	}
	return nil
}
...
func validateRegIdDomain(id string) error {
	...
	if err != nil {
		return fmt.Errorf(
			"invalid domain, %q must be formatted 'regId:domain': %s", id, err)
	}
	return nil
}
...
func validateFQDNSet(id string) error {
	...
	for _, domain := range domains {
		...
		if err != nil {
			return fmt.Errorf(
				"invalid domain, %q must be formatted 'fqdnSet': %s", id, err)
		}
	}
	return nil
}
