...
func Test_Limiter_initialization_via_Check_and_Spend(t *testing.T) {
	...
	d, err := l.Check(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 1)
	test.AssertNotError(t, err, "should not error")
	...
	d, err = l.Check(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 0)
	test.AssertNotError(t, err, "should not error")
	test.Assert(t, d.Allowed, "should be allowed")
	...
	// Reset our bucket.
	err = l.Reset(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne)
	test.AssertNotError(t, err, "should not error")
	...
	d, err = l.Spend(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 1)
	test.AssertNotError(t, err, "should not error")
	test.Assert(t, d.Allowed, "should be allowed")
	test.AssertEquals(t, d.Remaining, int64(19))
	// Verify our ResetIn timing is correct. 1 second == 1000 milliseconds and
...
func Test_Limiter_Check_bad_cost(t *testing.T) {
	l, _ := newTestLimiter(t)
	_, err := l.Check(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, -1)
	test.AssertErrorIs(t, err, ErrInvalidCostForCheck)
}

func Test_Limiter_Check_limit_no_exist(t *testing.T) {
	l, _ := newTestLimiter(t)
	_, err := l.Check(context.Background(), Name(9999), tenZeroZeroOne, 1)
	test.AssertError(t, err, "should error")
}
...
func Test_Limiter_with_defaults(t *testing.T) {
	...
	_, err := l.Spend(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 21)
	test.AssertErrorIs(t, err, ErrInvalidCostOverLimit)

	// Attempt to spend all 20 requests, this should succeed.
	d, err := l.Spend(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 20)
	test.AssertNotError(t, err, "should not error")
	test.Assert(t, d.Allowed, "should be allowed")
...
func Test_Limiter_with_new_clients(t *testing.T) {
	...
	d, err = l.Spend(context.Background(), NewRegistrationsPerIPAddress, "10.0.0.100", 1)
	test.AssertNotError(t, err, "should not error")
	test.Assert(t, d.Allowed, "should be allowed")
	test.AssertEquals(t, d.Remaining, int64(19))
	test.AssertEquals(t, d.RetryIn, time.Duration(0))
...
func Test_Limiter_Refund_and_Reset(t *testing.T) {
	...
	// Refund 10 requests.
	d, err = l.Refund(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 10)
	test.AssertNotError(t, err, "should not error")
	test.AssertEquals(t, d.Remaining, int64(10))

	// Spend 10 requests, this should succeed.
	d, err = l.Spend(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 10)
	test.AssertNotError(t, err, "should not error")
	test.Assert(t, d.Allowed, "should be allowed")
	test.AssertEquals(t, d.Remaining, int64(0))
	test.AssertEquals(t, d.ResetIn, time.Second)
	...
	d, err = l.Refund(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 1)
...
func Test_Limiter_Check_Spend_parity(t *testing.T) {
	...
	i, err := il.Check(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 1)
	test.AssertNotError(t, err, "should not error")
	j, err := jl.Spend(context.Background(), NewRegistrationsPerIPAddress, tenZeroZeroOne, 1)
	test.AssertNotError(t, err, "should not error")
