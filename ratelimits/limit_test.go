...
func TestLoadAndParseOverrideLimits(t *testing.T) {
	...
	firstEntryKey, err := newFQDNSetBucketKey(CertificatesPerFQDNSet, []identifier.ACMEIdentifier{identifier.NewDNS("example.com")})
	test.AssertNotError(t, err, "valid fqdnSet with one domain should not fail")
	secondEntryKey, err := newFQDNSetBucketKey(CertificatesPerFQDNSet, []identifier.ACMEIdentifier{identifier.NewDNS("example.com"), identifier.NewDNS("example.net")})
	test.AssertNotError(t, err, "valid fqdnSet with two domains should not fail")
	thirdEntryKey, err := newFQDNSetBucketKey(CertificatesPerFQDNSet, []identifier.ACMEIdentifier{identifier.NewDNS("example.com"), identifier.NewDNS("example.net"), identifier.NewDNS("example.org")})
