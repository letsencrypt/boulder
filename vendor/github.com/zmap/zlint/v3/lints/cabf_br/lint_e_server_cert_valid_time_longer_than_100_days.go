...
type sc081SecondDate100ServerCertValidityTooLong struct{}
...
func init() {
	lint.RegisterCertificateLint(&lint.CertificateLint{
		...
		Lint: NewSC081SecondDate100ServerCertValidityTooLong,
	})
}

func NewSC081SecondDate100ServerCertValidityTooLong() lint.LintInterface {
	return &sc081SecondDate100ServerCertValidityTooLong{}
}

func (l *sc081SecondDate100ServerCertValidityTooLong) CheckApplies(c *x509.Certificate) bool {
	return util.IsServerAuthCert(c) && !c.IsCA
}

func (l *sc081SecondDate100ServerCertValidityTooLong) Execute(c *x509.Certificate) *lint.LintResult {
	if util.GreaterThan(c, 100) {
		return &lint.LintResult{Status: lint.Error, Details: fmt.Sprintf("Certificate is issued on or after March 15, 2027 and has a validity of %.0f days", util.CertificateValidityInDays(c))}
	}
