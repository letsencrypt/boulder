...
type sc081ThirdDate46ServerCertValidityTooLong struct{}
...
func init() {
	lint.RegisterCertificateLint(&lint.CertificateLint{
		LintMetadata: lint.LintMetadata{
			Name:          "w_server_cert_valid_time_longer_than_46_days",
			Description:   "TLS server certificates issued on or after on or after March 15, 2029 00:00 GMT/UTC should not have a validity period greater than 46 days",
			Citation:      "https://github.com/cabforum/servercert/pull/553",
			Source:        lint.CABFBaselineRequirements,
			EffectiveDate: util.CABF_SC081_THIRD_MILESTONE,
		},
		Lint: NewSC081ThirdDate46ServerCertValidityTooLong,
	})
}

func NewSC081ThirdDate46ServerCertValidityTooLong() lint.LintInterface {
	return &sc081ThirdDate46ServerCertValidityTooLong{}
}

func (l *sc081ThirdDate46ServerCertValidityTooLong) CheckApplies(c *x509.Certificate) bool {
	return util.IsServerAuthCert(c) && !c.IsCA
}

func (l *sc081ThirdDate46ServerCertValidityTooLong) Execute(c *x509.Certificate) *lint.LintResult {
	if util.GreaterThan(c, 46) {
		return &lint.LintResult{Status: lint.Warn, Details: fmt.Sprintf("Certificate is issued on or after March 15, 2029 and has a validity of %.0f days", util.CertificateValidityInDays(c))}
	}
