...
func (l *sc081FirstDate200ServerCertValidityTooLong) Execute(c *x509.Certificate) *lint.LintResult {
	if util.GreaterThan(c, 200) {
		return &lint.LintResult{Status: lint.Error, Details: fmt.Sprintf("Certificate is issued on or after March 15, 2026 and has a validity of %.0f days", util.CertificateValidityInDays(c))}
	}
