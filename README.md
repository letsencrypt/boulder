...
We recommend setting git's [fsckObjects setting](https://groups.google.com/forum/#!topic/binary-transparency/f-BI4o8HZW0/discussion) for better integrity guarantees when getting updates.
...
Boulder requires an installation of libtool-ltdl, goose, SoftHSMv2, and MariaDB 10.1 to work correctly. If you want to save some trouble installing MariaDB and SoftHSM you can run them using Docker:

    docker-compose up -d bmysql bhsm

Also, Boulder requires Go 1.9 or above. This version may not be available in OS repositories. If so, you will have to install from https://golang.org/dl/. Add ```${GOPATH}/bin``` to your path.

Ubuntu:

    sudo apt-get install libltdl3-dev mariadb-server softhsm2

CentOS:

    sudo yum install libtool-ltdl-devel MariaDB-server MariaDB-client softhsm

Arch Linux:

    sudo pacman -S libtool mariadb softhsm --needed

OS X:

    brew install libtool mariadb softhsm

Edit /etc/hosts to add this line:

    127.0.0.1 boulder boulder-mysql boulder-wfe boulder-wfe2 boulder-ra boulder-va boulder-ca boulder-publisher boulder-sa

Resolve Go-dependencies, set up a database:

    ./test/setup.sh

**Note**: `setup.sh` calls `create_db.sh`, which uses the root MariaDB user, so if you have disabled that account or changed the password you may have to adjust the file or recreate the commands. It is also assumed that MariaDB is listening on the default port (`3306`).

Run ./test/make-softhsm.sh and follow its instructions to setup SoftHSM2 for key storage.
...
Run tests (this requires that the services are not already running as it will start them itself):

    ./test.sh

## Contributing

Please take a look at the [CONTRIBUTING.md](https://github.com/letsencrypt/boulder/blob/master/CONTRIBUTING.md) file for our guidelines on submitting patches, code review process, code of conduct, and various other tips on working on the codebase.
