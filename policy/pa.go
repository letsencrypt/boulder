...
var (
	...
	errICANNTLD             = berrors.MalformedError("Domain name ends with a reserved ICANN TLD. Certificate can not be issued")
	errPolicyForbidden      = berrors.RejectedIdentifierError("Policy forbids issuing a certificate for the domain name. Part or all of the name may be blacklisted")
	...
	errLabelTooLong         = berrors.MalformedError("Domain label too long. Each part of the domain name must be between 1 and 63 octets (bytes) long")
	...
	errICANNTLDWildcard     = berrors.MalformedError("Domain name is a wildcard for a reserved ICANN TLD. Certificate can not be issued")
