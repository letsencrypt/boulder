...
func TestCheckPrimeFactorsTooClose(t *testing.T) {
	...
	p, ok := big.NewInt(0).SetString("12451309173743450529024753538187635497858772172998414407116324997634262083672423797183640278969532658774374576700091736519352600717664126766443002156788367", 10)
	test.AssertEquals(t, ok, true)
	q, ok := big.NewInt(0).SetString("12451309173743450529024753538187635497858772172998414407116324997634262083672423797183640278969532658774374576700091736519352600717664126766443002156788337", 10)
	test.AssertEquals(t, ok, true)
	n = big.NewInt(0).Mul(p, q)
	test.AssertNotError(t, checkPrimeFactorsTooClose(n, 0), "factored n in too few iterations")
	err = checkPrimeFactorsTooClose(n, 1)
	test.AssertError(t, err, "failed to factor n")
	test.AssertContains(t, err.Error(), p.String())
	test.AssertContains(t, err.Error(), q.String())

	// These factors differ by slightly more than 2^256.
	p, ok = big.NewInt(0).SetString("11779932606551869095289494662458707049283241949932278009554252037480401854504909149712949171865707598142483830639739537075502512627849249573564209082969463", 10)
	test.AssertEquals(t, ok, true)
	q, ok = big.NewInt(0).SetString("11779932606551869095289494662458707049283241949932278009554252037480401854503793357623711855670284027157475142731886267090836872063809791989556295953329083", 10)
	test.AssertEquals(t, ok, true)
	n = big.NewInt(0).Mul(p, q)
	test.AssertNotError(t, checkPrimeFactorsTooClose(n, 13), "factored n in too few iterations")
