...
func TestCerficatesTableContainsDuplicateSerials(t *testing.T) {
	...
	// Register and insert and identical certificate to the 1337A.
	testCtx.setupCertificate1337B(t)

	// Despite a duplicate being present, this shouldn't error because the
	// serial and DER bytes of `certA`, `certB` are equivalent.
	...
	// different hostname. This should error because, despite sharing the same
	// serial, the DER bytes of `certA`, `certB`, and `CertC` are not
	// equivalent.
	testCtx.setupCertificate1337C(t)
...
// setupCertificate1337C registers and inserts a record into the `certificates`
// table with a domain of `1337.net`, `registrationID` of `1` and a `serial` of
// `1337`.
func (c testCtx) setupCertificate1337C(t *testing.T) {
	jsonKey := []byte(`
{
  "kty":"RSA",
