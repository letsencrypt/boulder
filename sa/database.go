...
var checkQuoteRE = regexp.MustCompile(`^'{1}\w+'{1}$`)
var checkImproperIntRE = regexp.MustCompile(`^'{1}\d+'{1}$`)
var checkFloatOrIntRE = regexp.MustCompile(`^?\d+(\.\d+)?$`)

// checkMariaDBSystemVariableQuoting ensures that a MariaDB config passed in via
// SA setDefault or via DSN have the correct quoting for string variables,
// enumerations as strings, and enumerations as integers/floating points to
// prevent future problems such as PR #6683 from occurring. This is a manually
// curated list of server variables generated by a tool in issue #6687.
func checkMariaDBSystemVariableQuoting(name string, value string) error {
	mariaDBStringTypes := []string{
		"bind_address",
	...
	for _, v := range mariaDBStringTypes {
		if name == v {
			if checkQuoteRE.FindString(value) == value {
				return nil
			}
	...
	for _, v := range mariaDBEnumTypes {
		if name == v {
			...
			} else if checkQuoteRE.FindString(value) == value {
				return nil
			}
	...
	// booleans or numerics (which can be either integers or floating points)
...
func adjustMariaDBConfig(conf *mysql.Config) error {
	...
	for k, v := range conf.Params {
		err := checkMariaDBSystemVariableQuoting(k, v)
		if err != nil {
			return err
		}
	}
