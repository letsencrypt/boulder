...
// checkMariaDBSystemVariables ensures that a MariaDB config passed in via SA
// setDefault or via DSN have the correct quoting for string variables,
// enumerations as strings, and enumerations as integers to prevent future
// problems such as PR #6683 from occurring. Additionally checks that only
// variables scoped as "session" rather than "global" have been defined. This is
// a manually curated list of server variables generated by a tool in issue
// #6687.
func checkMariaDBSystemVariables(name string, value string) error {
	// Create some hash sets that we'll index into to check for their existence.

	// Check MariaDB system variables are scoped as session. A session variable
	// change affects the current session only. Passing a session variable that
	// only exists in the global scope causes database connection error 1045.
	...
	// Keep in mind that certain MariaDB enums can have both string and integer
	// values.
	...
	if _, found := mariaDBSessionScope[name]; !found {
		return fmt.Errorf("%v is unable to be used in the session scope", name)
	}
	...
	if _, found := mariaDBIntEnumTypes[name]; found {
		if checkIntRE.FindString(value) == value {
			return nil
		} else if checkImproperIntRE.FindString(value) == value {
			return fmt.Errorf("%v=%v enumeration as integer is quoted, but should not be", name, value)
		}
	}
	...
	if _, found := mariaDBStringEnumTypes[name]; found {
		...
		return fmt.Errorf("%v=%v enumeration as string is not properly quoted", name, value)
	}

	// Finally, we don't perform any server variable validation checks against MariaDB
	// booleans or numerics (which can be either integers or floats)
	// each of which are represented as a Golang string type.
	if checkFloatOrIntRE.FindString(value) == value || value == "true" || value == "false" {
		return nil
	}

	return fmt.Errorf("variable %v was not found in the Boulder curated list of MariaDB server variables", name)
}
