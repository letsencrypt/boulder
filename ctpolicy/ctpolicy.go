...
func New(pub pubpb.PublisherClient, allLogs loglist.List, sctLogs loglist.List, infoLogs loglist.List, finalLogs loglist.List, stagger time.Duration, log blog.Logger, stats prometheus.Registerer) *CTPolicy {
...
// ExposeLoglistMetrics exposes several prometheus metrics generated by parsing
// the whole log list. Only needs to be called once because the loglist is not
// (currently) reloadable. There is no return value other than the scrapable
// prometheus metrics.
func (ctp *CTPolicy) ExposeLoglistMetrics() {
	// Boulder retrieves SCTs from logs in the sctLogs bucket. These logs have
	// been filtered deeper within the ctpolicy package. In the even that there's a log in the allLogs bucket, but
	for op, group := range ctp.sctLogs {
		ctp.opGroupSize.WithLabelValues(op, "sctLogs").Set(float64(len(group)))
		for _, log := range group {
			if log.EndExclusive.IsZero() {
				// Handles the case for non-temporally sharded logs too.
				ctp.shardExpiryTimestamp.WithLabelValues(op, log.Name).Set(float64(0))
			} else {
	...
	// allLogs contains all of the logs from the log list without any filtering
	// applied.
	for op, group := range ctp.allLogs {
		ctp.opGroupSize.WithLabelValues(op, "allLogs").Set(float64(len(group)))
	}

	for op, group := range ctp.infoLogs {
		ctp.opGroupSize.WithLabelValues(op, "infoLogs").Set(float64(len(group)))
	}

	for op, group := range ctp.finalLogs {
		ctp.opGroupSize.WithLabelValues(op, "finalLogs").Set(float64(len(group)))
	}
}
