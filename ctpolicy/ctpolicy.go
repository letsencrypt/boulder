...
// GetLoglistMetrics exports several prometheus metrics generated by parsing the
// entire log list. There is no return value other than the exported prometheus
// metrics.
func (ctp *CTPolicy) GetLoglistMetrics(ctx context.Context) {
	for op, group := range ctp.allLogs {
		groupSize := ctp.allLogs.GetGroupSize(op)
		ctp.opGroupSize.WithLabelValues(op).Set(groupSize)
		for _, log := range group {
			if log.EndExclusive.IsZero() {
				// Handles the case for non-temporally sharded logs.
