...
func New(pub pubpb.PublisherClient, sctLogs loglist.List, infoLogs loglist.List, finalLogs loglist.List, stagger time.Duration, log blog.Logger, stats prometheus.Registerer) *CTPolicy {
	...
	exposeLoglistMetrics(sctLogs, infoLogs, finalLogs, operatorGroupsGauge, shardExpiryGauge)
...
// the whole log list. Should only be called once because the loglist is not
// (currently) reloadable. There is no return value other than the scrapable
// prometheus metrics.
func exposeLoglistMetrics(sctLogs loglist.List, infoLogs loglist.List, finalLogs loglist.List, operatorGroupsGauge *prometheus.GaugeVec, shardExpiryGauge *prometheus.GaugeVec) {
