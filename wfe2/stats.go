...
type wfe2Stats struct {
	...
	// improperECFieldLengths counts the number of EC keys we see with
	// improper X and Y lengths for their curve
...
func initStats(scope metrics.Scope) wfe2Stats {
	...
	improperECFieldLengths := prometheus.NewCounter(
		prometheus.CounterOpts{
			...
			Help: "Number of EC keys with improper X and Y lengths",
		},
	)
